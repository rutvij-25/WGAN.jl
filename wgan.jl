...
function train_discriminator!(gen, dscr, x, opt_dscr, hparams)
    ...
    for i in 1:5
        ...
        for p in ps
            clamp!(p,-hparams.clip_val,hparams.clip_val)
        end
    end
    return loss
...
function train(; kws...)
    ...
    # Optimizers
    opt_dscr = RMSProp(hparams.lr)
    opt_gen = RMSProp(hparams.lr)
    ...
    output_image = create_output_image(gen, fixed_noise, hparams)
    save(@sprintf("output/wgan_steps_%06d.png", train_steps), output_image)
end
